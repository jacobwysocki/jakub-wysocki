...
/**
 * Pasek nawigacji prostego widoku.
 *
 * Hero jest jasne, więc biały pasek byłby nad nim niewidzialny: tonacja
 * jest ciemna od pierwszej klatki, a scroll przełącza już tylko podkład
...
export default function Nav({
  ...
  return (
    <>
      {/* Skip link jest ciemny w każdym stanie: pas, w którym się pojawia,
          leży nad jasnym hero, a niżej nad jasnymi sekcjami. */}
      <a
        href="#main"
        onClick={(e) => handleAnchor(e, "#main")}
        className="sr-only rounded-full bg-ink text-[13px] font-semibold text-white shadow-lift focus-visible:not-sr-only focus-visible:fixed focus-visible:left-6 focus-visible:top-4 focus-visible:z-[60] focus-visible:flex focus-visible:h-9 focus-visible:items-center focus-visible:px-5"
      >
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-4 sm:gap-6">
              <ul className="hidden items-center rounded-full border border-line/60 bg-surface/50 p-1 lg:flex">
                {navLinks.map((link) => (
                  <li key={link.href}>
                    <a
