...
const DOCK_APP_IDS = ["about", "experience", "studio", "contact"];
...
export default function MobileDesktop({ wallpaperId }: { wallpaperId: string }) {
  ...
  const desktopApps = APPS.filter((app) => app.desktopIcon);
  const dockApps = DOCK_APP_IDS.map((id) => getApp(id));
      ...
      >
        ...
        <header className="absolute inset-x-3 top-[max(env(safe-area-inset-top),0.75rem)] z-30">
          <div className="mx-auto flex h-11 max-w-xl items-center justify-between rounded-2xl border border-white/20 bg-black/25 px-2.5 text-white shadow-[0_12px_40px_rgba(0,0,0,0.18)] backdrop-blur-2xl">
            <div className="flex items-center gap-2 pl-1.5">
              ...
              <span className="text-[11px] font-bold tracking-[0.12em]">JW / OS</span>
            </div>
        ...
        >
          <div className="mx-auto max-w-xl">
            <motion.section
              ...
              transition={{ duration: reduced ? 0.15 : 0.65, ease: [...EASE_APPLE] }}
        ...
        >
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, scale: 1 }}
            transition={{ duration: reduced ? 0.15 : 0.65, delay: 0.22, ease: [...EASE_APPLE] }}
          ...
          >
            {dockApps.map((app) => (
              <motion.button
                ...
                onClick={() => api.openApp(app.id)}
                aria-label={`${t(ui.desktop.openApp)}: ${t(app.title)}`}
                className="relative flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl outline-none sm:h-12 sm:w-12"
